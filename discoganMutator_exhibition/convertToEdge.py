...
def image2edges(image):
    ...
    gray = cv2.cvtColor (image, cv2.COLOR_BGR2GRAY)
        ...
        # threshold
    ret,ThresPerim = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY)
...
for imagepath, name in zip(imagepathsA[start:number_of_images_to_edge], imageNamesA[start:number_of_images_to_edge]):
    
    image = cv2.imread(imagepath)
    edged = image2edges(image)

    cv2.imwrite(outputPathA_B + name + ".png", edged)
